...
void main()
{
	...
	vec3 ambient = 0.35 * vec3(1.0, 1.0, 1.0);
	vec3 diff = max(dot(normal, lightdir), 0.0)  * vec3(1.0, 1.0, 1.0);
	vec3 Spotlight = normalize(-fragPos);
	vec3 screen = reflect(-lightdir, normal);
	vec3 spec = pow(max(dot(Spotlight, screen), 0.0), 8.0) * vec3(1.0, 1.0, 1.0);
	float phongExp = 8.0;
	vec3 shade = ambient + diff + spec;
	vec3 binder = shade * texture2D(tex, vTexCoord).rgb;
	...
	if (isLightSource) {
        ...
        gl_FragColor = vec4(binder, 1.0); // Set the fragment color
    }
}
`;
